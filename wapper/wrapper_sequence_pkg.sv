        ...
        task body;
            repeat(50) begin
                ...
                assert(seq_item.randomize() with {
                    mosi_bits[10:8] inside {WA, WD};
                    if (last_cmd inside {WA, WD}) mosi_bits[10:8] inside {WA, WD};
                    else mosi_bits[10:8] == WA;
                });
                finish_item(seq_item);
                last_cmd = seq_item.mosi_bits[10:8];
            end
        endtask
    endclass
    ...
    class wrapper_read_only_sequence extends uvm_sequence #(wrapper_seq_item);
        ...
        task body;
            repeat(50) begin
                ...
                start_item(seq_item);
                assert(seq_item.randomize() with {
                    mosi_bits[10:8] inside {RA, RD};
                    if (last_cmd inside {RA, RD}) mosi_bits[10:8] inside {RA, RD};
                    else mosi_bits[10:8] == RA;
                });
                finish_item(seq_item);
                last_cmd = seq_item.mosi_bits[10:8];
            end
    ...
    class wrapper_write_read_sequence extends uvm_sequence #(wrapper_seq_item);
        ...
        task body;
            repeat(50) begin
                ...
                start_item(seq_item);
                assert(seq_item.randomize() with {
                    mosi_bits[10:8] inside {WA, WD, RA, RD};
                    if (last_cmd inside {WA, WD}) mosi_bits[10:8] inside {RA, RD};
                    else if (last_cmd inside {RA, RD}) mosi_bits[10:8] inside {WA, WD};
                    else mosi_bits[10:8] == WA;
                });
                finish_item(seq_item);
                last_cmd = seq_item.mosi_bits[10:8];
